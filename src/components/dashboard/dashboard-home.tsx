    ...
    <div className="max-w-5xl mx-auto space-y-6 pb-20 animate-fade-in">
        ...
        <Card className="border-parchment-700/20 dark:border-indigo-800/30 journal-shadow flex flex-col">
          ...
          <CardContent className="flex-1">
            <div className="space-y-3">
              {[{ topic: "AI", summary: "Open-source model releases are accelerating.", why: "Relevant to your work on Lurisa." }, { topic: "Entrepreneurship", summary: "Early-stage funding in African markets is shifting toward B2B SaaS.", why: "Aligned with your business interests." }, { topic: "Technology", summary: "Mobile AI infrastructure is becoming more accessible to indie builders.", why: "Could lower barriers for consumer apps." }].map((update, i) => (
                <div key={i} className="group cursor-default">
                  <div className="flex items-center gap-2 mb-0.5"><Lightbulb className="w-3 h-3 text-amber-500 shrink-0" /><p className="text-xs font-semibold text-charcoal-700 dark:text-parchment-100">{update.topic}</p></div>
                  <p className="text-xs text-charcoal-500 dark:text-parchment-300 leading-relaxed pl-5">{update.summary}</p>
                  <p className="text-[11px] text-charcoal-300/70 dark:text-parchment-400/70 pl-5 mt-0.5 italic">{update.why}</p>
                </div>
              ))}
            </div>
          </CardContent>
          <CardFooter className="pt-0 pb-5">
            <Link href="/insights" className="inline-flex items-center gap-1 text-xs font-semibold text-indigo-500 hover:text-indigo-700 dark:text-indigo-300 dark:hover:text-indigo-200 transition-colors group">Explore updates<ArrowRight className="w-3.5 h-3.5 group-hover:translate-x-0.5 transition-transform" /></Link>
          </CardFooter>
        </Card>
      </div>
      ...
      <Card className="border-terracotta-300/30 dark:border-terracotta-700/30 journal-shadow bg-terracotta-100/30 dark:bg-terracotta-900/10">
        <CardContent className="pt-8 pb-8 px-8 text-center">
          ...
          <p className="text-sm text-terracotta-600 dark:text-terracotta-300 mt-3 font-medium">— {todaysQuote.author}</p>
        </CardContent>
      </Card>
